...
function ExpensesPageContent() {
  ...
  // Fix: provide explicit initial value so TypeScript is happy
  ...
  const loadExpenses = useCallback(async () => {
    ...
    try {
      ...
      // Filter COGS client-side — no server param for it
  ...
  const expensesHeader = (
    <header className="mb-app-header">
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button className="mb-hdr-btn" aria-label="Filter by date"><Calendar size={17} /></button>
        <button className="mb-hdr-btn" aria-label="More filters"><SlidersHorizontal size={17} /></button>
      </div>
    </header>
  );
      ...
      <div style={{ padding: '14px var(--mb-page-x)', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
          <button
            ...
            onClick={() => { setActiveCatIds([]); setCogsOnly(false); }}
          >
            All
          </button>
          {pinned.map(cat => (
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[1, 2].map(g => (
              <div key={g}>
                <div className="mb-skeleton" style={{ height: 13, width: 110, marginBottom: 8, borderRadius: 6 }} />
