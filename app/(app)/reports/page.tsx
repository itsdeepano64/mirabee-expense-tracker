...
import { getCategoryBreakdown, getDashboardStats } from '@/lib/actions/expenses';
...
function getDateRange(range: RangeKey): { start: string; end: string } {
  ...
  if (range === 'quarter') {
    ...
    return {
      ...
      end: toISO(new Date(now.getFullYear(), q * 3 + 3, 0)),
    };
  }
  return { start: '', end: '' };
}
...
export default function ReportsPage() {
  ...
  const [stats, setStats] = useState<{ totalSpent: number; cogsTotal: number; receiptCount?: number } | null>(null);
  ...
  }, [range, customStart, customEnd, start, end]);
    ...
    <AppShell header={reportsHeader}>
      <div style={{ padding: '16px var(--mb-page-x)', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div className="mb-card-xl" style={{ padding: '18px 20px' }}>
          ...
          <div style={{
            display: 'flex', borderTop: '1px solid var(--mb-border)', paddingTop: 14,
          }}>
            {[
              { label: 'COGS', value: fmt(stats?.cogsTotal ?? 0), color: 'var(--mb-green)' },
              { label: 'Operating', value: fmt((stats?.totalSpent ?? 0) - (stats?.cogsTotal ?? 0)), color: 'var(--mb-blue)' },
              { label: 'Receipts', value: String(stats?.receiptCount ?? 0), color: 'var(--mb-text-muted)' },
            ].map((seg, i, arr) => (
          ...
          <div className="mb-card" style={{ padding: '14px 16px' }}>
            {loading ? (
              ...
              breakdown.map((item, i) => (
                <div key={item.category} style={{ marginBottom: i < breakdown.length - 1 ? 14 : 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--mb-text)' }}>{item.category}</span>
                    <div style={{ textAlign: 'right' }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--mb-text)' }}>{fmt(item.total)}</span>
                      <span style={{ fontSize: 10.5, color: 'var(--mb-text-muted)', marginLeft: 5 }}>{Math.round(item.percent)}%</span>
                    </div>
                  </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <ExportPdfButton start={start} end={end} className="mb-export-pdf" />
            <ExportCsvButton start={start} end={end} />
          </div>
        </div>
...
function ExportCsvButton({ start, end }: { start: string; end: string }) {
  async function handleCsv() {
    const { getExpenses } = await import('@/lib/actions/expenses');
    const exps = await getExpenses({ start, end, limit: 9999 });
    const rows = [
      ['Date', 'Description', 'Category', 'Amount', 'COGS', 'Notes'],
      ...exps.map(e => [
        e.date, e.description, e.category?.name ?? '',
    ...
    a.href = url;
    a.download = `mirabee-expenses-${start}-${end}.csv`;
    a.click();
