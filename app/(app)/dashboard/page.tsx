...
import { Plus, TrendingUp, Leaf, Tag, Hash } from 'lucide-react';
...
const CATEGORY_ICONS: Record<string, { icon: string; bg: string; color: string }> = {
  'Flowers & Plants': { icon: '🌸', bg: 'var(--mb-green-light)',  color: 'var(--mb-green)'      },
  'Wholesale Flowers': { icon: '🌷', bg: 'var(--mb-green-light)', color: 'var(--mb-green)'      },
  'Supplies':          { icon: '📦', bg: 'var(--mb-blue-xlight)', color: 'var(--mb-blue)'       },
  'Vases':             { icon: '🏺', bg: 'var(--mb-blue-xlight)', color: 'var(--mb-blue)'       },
  'Rent':              { icon: '🏠', bg: '#F6F0EA',               color: 'var(--mb-text-muted)' },
  'Utilities':         { icon: '⚡', bg: '#FEF9E7',               color: '#B7950B'              },
  'Marketing':         { icon: '📢', bg: 'var(--mb-pink-light)',  color: 'var(--mb-pink)'       },
  'Payroll':           { icon: '👥', bg: 'var(--mb-green-light)', color: 'var(--mb-green-dark)' },
  'Other':             { icon: '•',  bg: '#F6F0EA',               color: 'var(--mb-text-muted)' },
};

function catStyle(name: string | undefined | null) {
  return CATEGORY_ICONS[name ?? ''] ?? { icon: '•', bg: '#F6F0EA', color: 'var(--mb-text-muted)' };
}
    ...
    <AppShell header={dashHeader}>
      <div style={{ padding: '16px var(--mb-page-x)', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <span style={{
            fontSize: 11, fontWeight: 700, color: 'var(--mb-blue)',
            background: 'var(--mb-blue-xlight)', padding: '3px 10px',
        ...
        <div className="mb-spend-card">
          ...
          {stats && (
            <div style={{
              ...
              padding: '4px 11px', marginTop: 10,
              fontSize: 11.5, color: 'rgba(255,255,255,0.92)', fontWeight: 600,
            }}>
              <TrendingUp size={12} />
              {stats.vsLastMonth > 0 ? `Up ${stats.vsLastMonth}%` : `Down ${Math.abs(stats.vsLastMonth)}%`} from last month
            </div>
          )}

          <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
            <div className="mb-spend-pill">
              <div style={{ fontSize: 9.5, color: 'rgba(255,255,255,0.65)', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.04em', marginBottom: 3 }}>COGS</div>
              <div style={{ fontSize: 17, fontWeight: 700, color: '#fff' }}>{fmt(stats?.cogsTotal ?? 0)}</div>
            </div>
            <div className="mb-spend-pill">
              <div style={{ fontSize: 9.5, color: 'rgba(255,255,255,0.65)', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.04em', marginBottom: 3 }}>Expenses</div>
              <div style={{ fontSize: 17, fontWeight: 700, color: '#fff' }}>{stats?.expenseCount ?? 0}</div>
            </div>
            <div className="mb-spend-pill">
              <div style={{ fontSize: 9.5, color: 'rgba(255,255,255,0.65)', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '0.04em', marginBottom: 3 }}>Avg</div>
              <div style={{ fontSize: 17, fontWeight: 700, color: '#fff' }}>{fmt(stats?.avgExpense ?? 0)}</div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="mb-card" style={{ overflow: 'hidden' }}>
              {recent.length === 0 && (
                <div style={{ padding: '24px', textAlign: 'center', color: 'var(--mb-text-muted)', fontSize: 14 }}>
                  No expenses yet. Tap + to add one.
                </div>
              )}
              {recent.map((exp, i) => {
                const cs = catStyle(exp.category?.name);
                return (
                  <div
                    key={exp.id}
                    style={{
                  ...
                  >
                    <div className="mb-exp-icon" style={{ background: cs.bg }}>
                      <span style={{ fontSize: 18 }}>{cs.icon}</span>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--mb-text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {exp.description}
                      </div>
  ...
  return (
    <div className="mb-card" style={{ padding: '13px 14px' }}>
      <div style={{
        ...
        color: iconColor, marginBottom: 9,
      }}>
        {icon}
      </div>
      {loading ? (
      ...
      <div style={{ fontSize: 10.5, color: 'var(--mb-text-muted)', fontWeight: 500, marginTop: 4 }}>
        {label}
      </div>
    </div>
  );
